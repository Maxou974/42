...
{
	ft::map<int, string> v_i;

	for (int i = 0; i < 1000; i++)
		v_i.insert(ft::make_pair(i, "wyhefiwef"));
}
{
	header_test("ITERATOR");
	ft::map<int, string> v_i;

	v_i.insert(ft::make_pair(1, "qdgviy"));
	v_i.insert(ft::make_pair(2, "rfeoubg"));
	v_i.insert(ft::make_pair(0, "woeif"));

	ft::map<int,string>::iterator it = v_i.begin();
	ft::map<int, string>::const_iterator c_it = v_i.begin();
...
}
{
	...
	ft::map<int,string>::reverse_iterator it = v_i.rbegin();
	ft::map<int, string>::const_reverse_iterator c_it = v_i.rbegin();
