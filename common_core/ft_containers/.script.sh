...
if grep "no leaks are possible" output/std.valgrind > /dev/null && grep "0 context" output/std.valgrind > /dev/null
...
echo -n ${NOCOLOR}
if grep "no leaks are possible" output/ft.valgrind > /dev/null && grep "0 context" output/ft.valgrind > /dev/null
then echo "${GREEN}No errors\nNo leaks${NOCOLOR}" ; echo -n ${PURPLE} ; /usr/bin/time --format="time: %es  CPU: %P" ./a.out > /dev/null ; echo ${NOCOLOR}
